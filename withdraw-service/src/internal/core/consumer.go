...
func NewConsumer(brokerList string, topicList []string, groupID string, db *DBPool) *KafkaConsumer {
	...
	for {
		...
		case *kafka.Message:
			go func() {
				...
				if err := kc.Db.ProcessTransaction(transaction); err != nil {
					kc.Db.ChangeStatus(transaction, "Success")
					log.Printf("Sucess transaction: %s", transaction.ID)
				} else {
					kc.Db.ChangeStatus(transaction, "Error")
					log.Printf("Error transaction: %s", transaction.ID)
				}
			}()
		case kafka.Error:
